...
import { managerDraftSchema, managerFormSchema } from "@/lib/validations/manager-form.schema";
...
export async function POST(
  ...
  try {
    ...
    const { draft } = body;
    const parsed = (draft ? managerDraftSchema : managerFormSchema).safeParse(body);
    if (!parsed.success) {
    ...
    if (draft) {
      const updated = await prisma.appraisalSubmission.update({
        where: { id: params.id },
        data: { ...mgrData, managerDraftSavedAt: new Date() },
      });
      return NextResponse.json(updated);
    }

    const updated = await transitionStage(params.id, 1, 2, { ...mgrData, managerDraftSavedAt: null });
    return NextResponse.json(updated);
  } catch (error) {
    console.error(error);
